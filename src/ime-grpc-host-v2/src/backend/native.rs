...
use crate::proto::rime_service_v2::{RimeContextProto, KeyEvent, CompositionProto, MenuProto, CandidateProto};
use super::rime_ffi;
    ...
    pub fn new() -> Self {
        unsafe {
            ...
            let mut traits = rime_ffi::RimeTraits {
                data_size: std::mem::size_of::<rime_ffi::RimeTraits>() as libc::c_int - std::mem::size_of::<libc::c_int>() as libc::c_int,
        ...
        unsafe {
            ...
            if ((*self.api).get_context)(session_id as rime_ffi::RimeSessionId, &mut ctx) != 0 {
                ...
                if !ctx.composition.preedit.is_null() {
                    let preedit = CStr::from_ptr(ctx.composition.preedit).to_string_lossy().into_owned();
                ...
                if ctx.menu.num_candidates > 0 {
                    ...
                    let cand_array = std::slice::from_raw_parts(ctx.menu.candidates, ctx.menu.num_candidates as usize);
                    ...
                    menu_proto = Some(MenuProto {
                        ...
                        select_keys: if !ctx.menu.select_keys.is_null() {
                            CStr::from_ptr(ctx.menu.select_keys).to_string_lossy().into_owned()
                        } else {
                            String::new()
                        },
                    });
                ...
                let commit_text_preview = if !ctx.commit_text_preview.is_null() {
                    CStr::from_ptr(ctx.commit_text_preview).to_string_lossy().into_owned()
                } else {
                    String::new()
                };
