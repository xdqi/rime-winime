...
impl ChannelRimeBackend {
    pub fn new(mut inner: Option<Box<dyn RimeBackend>>) -> Self {
        ...
        let _ = thread::spawn(move || {
            let mut runtime_backend = inner
                .take()
                .unwrap_or_else(|| Box::new(crate::win_imm::ImmRimeAdapter::default()));
            ...
            for cmd in rx {
                match cmd {
                    ...
                    BackendCommand::SelectCandidate {
                        session_id,
                        index,
                        reply,
                    } => {
                        let _ = reply
                            .send(rt.block_on(runtime_backend.select_candidate(session_id, index)));
                    }
                }
            }
        });
...
impl RimeBackend for ChannelRimeBackend {
    async fn open_session(&mut self) -> Option<usize> {
        ...
        self.tx
            .send(BackendCommand::OpenSession { reply: tx })
            .unwrap();
    ...
    async fn process_key(&mut self, session_id: usize, key: &KeyEvent) -> bool {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(BackendCommand::ProcessKey {
                session_id,
                key: *key,
                reply: tx,
            })
    ...
    async fn get_context(&mut self, session_id: usize) -> RimeContextProto {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(BackendCommand::GetContext {
                session_id,
                reply: tx,
            })
            .unwrap();
    ...
    async fn select_candidate(&mut self, session_id: usize, index: usize) -> bool {
        let (tx, rx) = oneshot::channel();
        self.tx
            .send(BackendCommand::SelectCandidate {
                session_id,
                index,
                reply: tx,
            })
