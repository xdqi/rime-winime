...
use ime_grpc_host_v2::proto::rime_service_v2::{
    ...
    DestroySessionRequest, OpenSessionRequest, ProcessKeyRequest, KeyEvent,
};
...
async fn open_session(client: &mut RimeServiceClient<tonic::transport::Channel>) -> String {
    let resp = client.open_session(OpenSessionRequest { schema_id: String::new() }).await
...
async fn destroy_session(client: &mut RimeServiceClient<tonic::transport::Channel>, session_id: &str) {
    let _ = client.destroy_session(DestroySessionRequest { session_id: session_id.to_string() }).await;
...
async fn process_key(
    ...
    let resp = client.process_key(ProcessKeyRequest {
        session_id: session_id.to_string(),
        key_event: Some(KeyEvent { keycode, modifier }),
    }).await.expect("ProcessKey RPC failed");
...
async fn get_commit(
    ...
    let resp = client.get_commit(GetCommitRequest { session_id: session_id.to_string() }).await
...
async fn get_context_preedit(
    ...
    let resp = client.get_context(GetContextRequest { session_id: session_id.to_string() }).await
...
async fn test_grpc_nihao_comma() {
    ...
    println!("\n=== Connecting to {} ===", addr);
    let mut client = RimeServiceClient::connect(addr).await
    ...
    let keys = [
        (0x6E, 0u32, 'n'), (0x69, 0, 'i'), (0x68, 0, 'h'), (0x61, 0, 'a'), (0x6F, 0, 'o'),
        (0x20, 0, ' '),
    ];
    let commit = run_key_sequence(&mut client, &session_id, &keys).await;
    println!("  Total commit: {:?}", commit);
    assert!(commit.contains("你好"), "Expected '你好' in commit, got {:?}", commit);
    ...
    println!("\n--- Test: nihao,nihao, then standalone comma (replicate rime_api_console) ---");
    // First "nihao," — should just create composition
    let keys1 = [
        (0x6E, 0u32, 'n'), (0x69, 0, 'i'), (0x68, 0, 'h'), (0x61, 0, 'a'), (0x6F, 0, 'o'),
        (0x2C, 0, ','),
    ];
    let commit1 = run_key_sequence(&mut client, &session_id, &keys1).await;
    println!("  After first 'nihao,': commit={:?}", commit1);
    ...
    let commit2 = run_key_sequence(&mut client, &session_id, &keys2).await;
    println!("  After second 'nihao,': commit={:?}", commit2);

    // Standalone comma
    let keys3 = [(0x2C, 0u32, ',')];
    let commit3 = run_key_sequence(&mut client, &session_id, &keys3).await;
    println!("  After standalone comma: commit={:?}", commit3);

    println!("  Combined: {:?}", format!("{}{}{}", commit1, commit2, commit3));
