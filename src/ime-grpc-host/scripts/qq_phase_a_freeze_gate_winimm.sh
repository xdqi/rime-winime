...
RUN_LOG=".cache/qq-phase-a-gate-winimm-run-${STAMP}.log"
HOST_LOG=".cache/qq-phase-a-gate-winimm-host-${STAMP}.log"
CHECK_LOG=".cache/qq-phase-a-gate-winimm-check-${STAMP}.log"
...
if [[ ! -x "target/debug/phase_a_contract_checker" ]]; then
  log_line "[build] cargo build --bin phase_a_contract_checker"
  cargo build --bin phase_a_contract_checker 2>&1 | tee -a "${RUN_LOG}"
fi

if [[ ! -x "target/i686-pc-windows-gnu/debug/ime-grpc-host.exe" ]]; then
  log_line "[build] cargo build --target i686-pc-windows-gnu --bin ime-grpc-host"
  cargo build --target i686-pc-windows-gnu --bin ime-grpc-host 2>&1 | tee -a "${RUN_LOG}"
fi
...
log_line "[host] start port=${PORT} backend=win_imm dll=${DLL_PATH}"
WINEPREFIX="${WINE_PREFIX}" WINEDEBUG=-all RUST_LOG="${RUST_LOG:-}" IME_GRPC_BIND="127.0.0.1:${PORT}" IME_WORKER_BACKEND=win_imm IME_WINIMM_FORCE_REAL=1 IME_WINIMM_DLL="${DLL_PATH}" wine target/i686-pc-windows-gnu/debug/ime-grpc-host.exe > >(tee -a "${HOST_LOG}") 2>&1 &
HOST_PID=$!
